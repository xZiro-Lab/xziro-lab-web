...
import { ArrowRight, Github, Star, GitFork, Users, Code2 } from "lucide-react";
        ...
        <div className="container relative z-10">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 mb-6 px-3 py-1 bg-accent/10 border border-accent/30 rounded">
              ...
              <span className="text-sm font-mono text-accent">Open Source AI Research</span>
            </div>
            ...
            <p className="text-xl text-muted-foreground mb-8 max-w-2xl">
              Exploring cutting-edge AI research through open source. Building experimental prototypes and pushing the boundaries of what's possible.
            </p>
        ...
        <div className="container">
          <div className="mb-16">
            ...
            <p className="text-lg text-muted-foreground">
              Active and experimental AI research projects
            </p>
          </div>

          <div className="grid md:grid-cols-2 gap-8 mb-16">
            {featuredProjects.map((project) => (
              <Card key={project.id} className="bg-background border-border hover:border-accent/50 transition-all duration-300 overflow-hidden group">
                ...
                <CardContent className="pt-6">
                  <h3 className="text-2xl font-bold text-foreground mb-2 font-mono">{project.name}</h3>
      ...
      <section className="py-20 md:py-32 bg-card border-t border-b border-border">
        <div className="container">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold text-foreground mb-4 font-mono">Research Areas</h2>
            <p className="text-lg text-muted-foreground">
              Exploring diverse domains of AI research
            </p>
          </div>

          <div className="grid md:grid-cols-3 gap-6">
            {categories.map((category: any) => (
              <Card key={category.id} className="bg-background border-border hover:border-accent/50 transition-all duration-300">
                <CardContent className="pt-8 text-center">
                  <div className="text-5xl mb-4">{category.icon}</div>
                  <h3 className="text-lg font-bold text-foreground mb-2 font-mono">{category.label}</h3>
                  <p className="text-muted-foreground text-sm">
                    {projects.filter((p: Project) => p.category === category.id).length} project{projects.filter((p: Project) => p.category === category.id).length !== 1 ? 's' : ''}
                  </p>
                </CardContent>
              </Card>
            ))}
      ...
      <section className="py-20 md:py-32">
        <div className="container">
          <div className="max-w-3xl mx-auto text-center">
            <h2 className="text-4xl font-bold text-foreground mb-6 font-mono">Join the Research</h2>
            <p className="text-xl text-muted-foreground mb-8">
              Interested in contributing to open source AI research? Check out our projects on GitHub and join the community.
            </p>
