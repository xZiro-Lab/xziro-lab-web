...
export const stats = {
  ...
  totalStars: projects.reduce((sum: number, p: Project) => sum + p.metrics.stars, 0),
  totalContributors: projects.reduce((sum: number, p: Project) => sum + p.metrics.contributors, 0),
  totalCommits: projects.reduce((sum: number, p: Project) => sum + p.metrics.commits, 0),
};
